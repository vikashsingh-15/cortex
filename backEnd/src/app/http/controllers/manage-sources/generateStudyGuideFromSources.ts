...
export async function generateStudyFromSources(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (studyguides.length > 0) {
            ...
            if (studyguides.length === 1) {
                ...
                const title = `Study guide · ${studyguides[0]?.title || "Source"}`;
                ...
                return res.status(200).send({ message: "finished creating studyguide" })
                ...
                const title = `Study guide · ${countSource} sources`;

                await sourceRepo.createSource({
                    userId, noteId,
                    title,
                    source_type: 'studyguide',
                    content: llmFinalstudyguide,
